...
export class UsuarioService {
  ...
  //     this.getcorreoBy(user.email).subscribe(data => {
  //       const correoEnsayo = data[0]?.email;

  //       if (correoEnsayo !== undefined) {
  //         console.log(correoEnsayo)
  //         // Almacena la cadena JSON en sessionStorage
  //         sessionStorage.setItem('userData', JSON.stringify(userDataString));
  //         this.router.navigate(['/list-I']);
  //       } else {
  //         this.errores.showErrorCenter('Correo inválido');
  //       }
  //     });
  //   } catch (error) {
  //     console.log(error);
  ...
  // }
  // getcorreoBy(email: string): Observable<any> {
  //   return this.firestore.collection('userInfo', ref => ref.where('email', '==', email)).valueChanges();
  // }
  loginGoogle() {
    return this.authLogin(new GoogleAuthProvider());
  }
